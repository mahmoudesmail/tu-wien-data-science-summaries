...
This survey paper explores the interactive visualization design of graphs with two specific properties: time-dependency and bipartite structure. Time-dependent graphs represent how data changes over time, allowing us to observe patterns, trends, and anomalies across different time periods. Bipartite graphs, on the other hand, represent relationships between two distinct sets of entities, with connections only existing between nodes from different groups (not within the same group).

We use the ``Media Transparency Database Austria'' \cite{dataset} as a practical example to illustrate these concepts. This public repository records advertisement expenses of public authorities in Austria, providing a rich source of information about how government bodies allocate their advertising budgets over time and across different media outlets.
...
Time-dependent graphs are visual representations that show how data changes over time[1]. They allow us to observe patterns, trends, and anomalies across different time periods. These graphs are particularly useful for analyzing dynamic systems or processes that evolve over time.

Bipartite graphs, on the other hand, represent relationships between two distinct sets of entities[3]. In a bipartite graph, nodes are divided into two groups, and connections only exist between nodes from different groups. This type of graph is ideal for visualizing interactions or associations between two categories of data.

The Media Transparency Database Austria is a public repository that records advertisement expenses of public authorities in Austria. This database provides a rich source of information about how government bodies allocate their advertising budgets over time and across different media outlets.

Interactive visualization enhances the exploration of such complex datasets. It allows users to dynamically adjust the display, filter data, and zoom in on specific areas of interest[2]. This interactivity enables deeper insights and more efficient data analysis.

By applying time-dependent and bipartite graph visualizations to the Media Transparency Database, we can uncover valuable insights. For instance, we can observe how advertising spending patterns change over time, which public authorities are the biggest advertisers, and which media outlets receive the most funding.

This survey paper aims to explore various techniques and approaches for designing interactive visualizations of time-dependent and bipartite graphs. We will examine different methods for representing temporal data, strategies for layouting bipartite relationships, and ways to combine these approaches effectively.

Throughout this paper, we will focus on creating clear, uncluttered overviews while maximizing information content[3]. We will also discuss techniques for handling large-scale data, as the Media Transparency Database likely contains thousands of entries over multiple years.

By the end of this survey, readers will have a comprehensive understanding of how to design effective interactive visualizations for time-dependent and bipartite graphs, using the Media Transparency Database Austria as a practical and relevant example.
...
Data visualization is crucial for understanding complex relationships. Time-dependent bipartite graphs represent evolving connections between two distinct entity sets. These graphs are powerful but challenging to visualize effectively.

This research focuses on developing interactive visualization techniques for time-dependent bipartite graphs. We use the Media Transparency Database Austria as our case study. This database records advertising expenditures from Austrian public authorities to media companies over time.
...
The goal is to create accessible, insightful visualizations for both experts and non-experts. Our findings will contribute to information visualization research and provide tools for analyzing public spending on advertising in Austria.





\subsection{Time-dependent bipartite graphs}
...
% https://www.statistik.at/en/services/tools/services/transparency-database-evaluations


% Interactive Visualization design of time-dependent and bipartite graphs using the example of the Media Transparency Database Austria.
...
= a public database of advertisement expenses of public authorities in Austria
...
% Time dependency of graphs -> Time dependency is interesting - for example to understand how advertising activities change after changes in government and how the media landscape changes over time.
...
% Design Justification / Hypothesis Building (part of research process)
